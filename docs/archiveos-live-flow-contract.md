...
| `GET` | `/api/runtime-events/recent?after={cursor}&limit=100` | 마지막 cursor 이후의 새 runtime event 조회 |
...
각 runtime event는 `eventId`, `idempotencyKey`, `sourceService`, `targetService`, `domain`, `eventType`, `entityType`, `entityId`, `correlationId`, `causationId`, `simulationRunId`, `settlementCycleId`, `workdayId`, `status`, `severity`, `occurredAt`, `hopCount`, `maxHop`, `cursor`, `metadata`를 제공한다. ArchiveOS는 마지막 `cursor`를 저장해 증분 수집한다.
...
| `SETTLEMENT_STARTED` | capacity가 배정된 정산 batch 시작 |
...
Archive-Ledger는 현재 검증된 ArchiveOS ingest 인증 계약을 보유하지 않아 외부 push outbox를 운영하지 않는다. 따라서 `/api/operations/summary`의 `outbox` 값은 계약 호환을 위해 0으로 노출하며, ArchiveOS는 cursor 기반 pull을 사용한다.
